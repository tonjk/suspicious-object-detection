...
def detect_abandoned(
        video_path: str,
        output_path: str = "video_output/output.mp4",
        preview: bool = True,
        max_frames: int = 0,
        capture_dir: str | None = None,
        yield_intermediate: bool = False,
        save_video: bool = False,
) -> tuple[str | None, list[str]] | Iterator[tuple[object, object]]:

    # Sync path must not rely on StopIteration.value (can be wrong in some Python /
    # generator edge cases). Store the tuple explicitly instead.
    _sync_result: tuple[str | None, list[str]] | None = None
    ...
    def _run():
        ...
        try:
            while cap.isOpened():
                ...
                tracked_bag_centroids = [tr.centroid for tr in bag_tracks.values()]
                ...
                for box in results.boxes:
                    ...
                    if label == PERSON_CLASS and conf >= PERSON_CONF:
                        ...
                        is_near_tracked = any(
                            ...
                            for tc in tracked_bag_centroids
                        )
                ...
                # visible_bags = {bid for bid in current_bag_ids if bid in bag_tracks}

                abandoned_count = 0
                bids_to_capture_this_frame = []
                ...
                for bid in current_bag_ids:
                    ...
                    if owner_is_near:
                        ...
                        if bag.is_abandoned:
                            ...
                            color = (0, 0, 255)
                            abandoned_count += 1
                            if capture_dir and bid not in captured_bids:
                                bids_to_capture_this_frame.append(bid)
                ...
                hud = (
                    f"Frame: {frame_idx}/{total_frames} ({progress_pct}%)  "
                    f"Persons: {len(visible_people)}  Bags: {len(current_bag_ids)}  Abandoned: {abandoned_count}"
                )
                cv2.putText(frame, hud, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)

                for bid in bids_to_capture_this_frame:
                    capture_path = os.path.join(capture_dir, f"abandoned_{bid}_{frame_idx}.jpg")
                ...
                # --- Yield annotated frame for Gradio real-time display ---
                if yield_intermediate and (frame_idx % YIELD_EVERY_N_FRAMES == 0):
                    # Convert BGR -> RGB for Gradio Image component
                    frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                
                    # Resize downstream frame to improve UI performance and WebSocket speed
                    ...
                    yield frame_rgb, captured_frames.copy()
        ...
        if yield_intermediate:
            # Final yield: send None frame to signal completion + full capture list
            yield None, captured_frames
        else:
            if save_video:
                print(f"[INFO] Done. Saved -> {output_path}")
            else:
                print("[INFO] Done. No video saved.")
            _sync_result = (output_path if save_video else None, captured_frames)
            return
    ...
    if _sync_result is None:
        raise RuntimeError("detect_abandoned finished without a result (unexpected generator exit)")
    return _sync_result
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="DIR",
        help="Save JPG snapshots when a bag first becomes abandoned (default: no snapshots)",
    )
    ...
    output_path, captures = detect_abandoned(
    ...
    if captures:
        print(f"[INFO] Captured {len(captures)} snapshot(s) under {args.capture_dir!r}")